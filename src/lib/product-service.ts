...
import { generateProductDescription } from '@/ai/flows/visual-search-flow';

// Type for creating a new product (omitting fields generated by server)
type ProductInput = Omit<Product, 'id' | 'rating' | 'reviews' | 'isNew' | 'createdAt'>;
...
export const createProduct = async (productData: Partial<ProductInput>) => {
  ...
  const aiDescription = productData.images?.[0] 
    ? await generateProductDescription({ photoDataUri: productData.images[0] }) 
    : '';
...
export const updateProduct = async (productId: string, productData: Partial<Product>) => {
  ...
  try {
    ...
    // If images are updated, regenerate AI description
    if (productData.images && productData.images.length > 0) {
        const currentProduct = await getProductById(productId);
        if (currentProduct && productData.images[0] !== currentProduct.images[0]) {
             dataToUpdate.aiDescription = await generateProductDescription({ photoDataUri: productData.images[0] });
        }
    }
